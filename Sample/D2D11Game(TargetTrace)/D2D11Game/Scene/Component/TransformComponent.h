...
public:
	...
	bool HasParent() const { return parent.expired() != true ? true : false; }
	bool HasChilds() const;

	TransformComponent* const GetRoot() { return HasParent() == true ? parent.lock()->GetRoot() : this; }

	std::shared_ptr<TransformComponent> const GetParent() { return parent.lock(); }
	TransformComponent* const GetParent_Raw() { return parent.lock().get(); }
	void SetParent(std::shared_ptr<TransformComponent> const new_parent);

	const std::vector<std::weak_ptr<TransformComponent>>& GetChilds() const { return childs; }

	//std::shared_ptr<TransformComponent> const GetChildFromIndex(const uint& index);
	uint GetChildCount() const;

	void AddChild(std::shared_ptr<TransformComponent> const child);
