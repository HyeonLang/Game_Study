...
Player::Player(Float2 pos, Float2 size, Float2 s, float a, Matrix* parent)
	: Transform(pos, s, a, parent), prev_pos(pos)
{
	...
	collider = new RectCollider(Vector2());
}
...
void Player::Update(DWORD* deltatime, Float2 target)
{
	if (*deltatime > 17) {
		if (key_map != 0) // 현재 눌려있는 키가 하나라도 있는 경우
		{
	...
	if (prev_pos.x != target.x || prev_pos.y != target.y)	// 직전 마우스 위치와 현재가 일치하지 않으면
	{
		prev_pos = target;
