...
/*
Player 쪽은 대충 마무리가 됐는데,
한 가지 재밌는 것 : 적이 플레이어를 곧바로 쫒아오도록 만들어보고 싶다
그걸 위해 필요한 것은 다음과 같음

1. 어떻게든 플레이어의 위치를 받을 수단이 필요
2. 플레이어를 쫓는 적이 있다 하더라도 기존의 무작위로 움직이는 적을 없앨 필요는 없음
그렇다고 새 클래스를 따로 만들어야 하는 것도 아니니, 가급적 이 클래스를 재활용하고 싶음

-> 1. 플레이어의 위치를 업데이트받을 때마다 전달하면 그만
2. 이 적 오브젝트가 어떤 타입인지를 관리하는 변수를 만들어서
그 변수에 맞게 행동시키도록 코드를 짜면 그만
...
void Enemy::Update(DWORD time, Float2* target)
{
	...
	// ImGui를 이용해서 실제로 회전하는 각이 어떤 식으로 작동하는지 체크해보니
	// 위를 바라보는 것이 0도로 취급되고 있고,
	// 시계 반대방향으로 갈 때(수학에서의 보편적인 좌표계)가 아닌
	// 시계방향으로 갈 때 회전값이 늘어나는 것을 볼 수 있었음
	// 그에 따라 삼각함수 계산을 다시 해보면 
