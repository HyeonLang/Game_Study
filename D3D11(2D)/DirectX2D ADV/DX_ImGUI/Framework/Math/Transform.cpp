	...
	: pos(p), scale(s), angle(a), parent(parent)
{
	...
	WB = new MatrixBuffer();	
	// Scene이 아니라 각각 오브젝트가 스스로 월드변환이 가능하도록 하는 출력에 사용할 월드 버퍼
}
...
void Transform::WorldUpdate()
{
	...
	XMStoreFloat4x4(&mat_world, world);	// 4x4 행렬에서 4x4 행렬로 데이터 복사
	XMVECTOR outS, outR, outT;	// 변환 완료 월드 행렬에서 추출한 Scale, Rotation, Transform 변수
	XMMatrixDecompose(&outS, &outR, &outT, world);	// 월드 행렬에서 추출, 정해진 자료형을 통해서만 사용
	XMStoreFloat2(&global_scale, outS);	// float 2개로 XMMatrixDecompose 같은 동작
}
